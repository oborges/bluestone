...
func TestNFSv4LockThroughCompound(t *testing.T) {
	srv, handler, _ := newNFS4TestServer(t)
	// A nil NFSv4Locker disables locking here, so script one.
	locker := &fakeLocker{}
	srv.NFSv4Locker = locker
	unchecked := &nfs4CreateHow{Mode: nfs4CreateUnchecked}
	...
	locker.conflict = &LockConflict{LockRange: LockRange{Start: 0, End: 100, Exclusive: true}, Owner: nfs4Owner{ClientID: 1, Owner: "alice"}.lockerOwner()}
