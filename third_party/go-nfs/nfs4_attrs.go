...
var (
	...
	nfs4WritableAttrs = nfs4BitmapOf(nfs4AttrSize, nfs4AttrMode, nfs4AttrOwner, nfs4AttrOwnerGroup, nfs4AttrTimeAccessSet, nfs4AttrTimeModifySet)
)
...
func nfs4DecodeSetAttrs(fattr nfs4FAttr) (nfs4SetAttrs, nfs4Status) {
	...
	for _, id := range fattr.Mask.attrs() {
		...
		case nfs4AttrOwner, nfs4AttrOwnerGroup:
			var owner string
			if err := xdrReadLimited(vals, &owner, nfs4OpaqueLimit); err != nil {
				return nfs4SetAttrs{}, nfs4ErrBadXDR
			}
			ownerID, ok := parseNFS4Owner(owner)
			if !ok {
				return nfs4SetAttrs{}, nfs4ErrBadOwner
			}
			if id == nfs4AttrOwner {
				attrs.attrs.SetUID = &ownerID
			} else {
				attrs.attrs.SetGID = &ownerID
			}
