...
func nfs4OnOpen(c *nfs4Compound, args io.Reader, res io.Writer) nfs4Status {
	...
	existed := statErr == nil
	if existed {
		read, write := req.ShareAccess&nfs4ShareAccessRead != 0, req.ShareAccess&nfs4ShareAccessWrite != 0
		if err := checkOpen(c.w, parent.fs, childPath, read, write); err != nil {
			return nfs4StatusFromErr(err)
		}
	} else if req.OpenType == nfs4OpenCreate {
		if err := checkParent(c.w, parent.fs, childPath); err != nil {
			return nfs4StatusFromErr(err)
		}
	}
	if req.OpenType == nfs4OpenCreate {
		if existed && req.How.Mode != nfs4CreateUnchecked {
			return nfs4ErrExist
		}
		...
		if err := checkSetAttr(c.w, parent.fs, childPath, &createAttrs.attrs); err != nil {
			return nfs4StatusFromErr(err)
		}
		...
		return nfs4StatusFromErr(statErr)
	}
